...
export class AnswerService {
    ...
    async getSurveyAnswersById(surveyId:number) : Promise<IAnswerRO[]>  {
        const answersArray = []
    ...
    async updateOneAnswer( dto: UpdateAnswerDto,surveyId:number): Promise<IAnswerRO> {
        ...
        await this.answerEntityRepository.update(toUpdate.id, {
            ...(dto.body && { body: dto.body }),
            ...(dto.position && { position: dto.position }),
            ...(dto.ended && { ended: dto.ended }),
            ...({ lastUpdateDate:new Date() }),
        });


        return this.buildAnswerRO(updated);
    }
