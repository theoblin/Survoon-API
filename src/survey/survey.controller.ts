...
export class SurveyController {
  constructor(private readonly surveyService: SurveyService,private readonly answerService: answerService.AnswerService) {}
    ...
    @ApiResponse({ status: 403, description: 'Forbidden.' })
    @Put() 
    async updateOneSurvey(@User('id') userId: number,@Body('survey') newSurveyData: UpdateSurveyDto){
      return this.surveyService.updateOneSurvey(newSurveyData,userId)
    }
    ...
    @ApiResponse({ status: 403, description: 'Forbidden.' })
    @Delete("/:id") 
    async deleteOneSurvey(@Param() data){
      return this.surveyService.deleteSurveyById(data.id)
    }
